...
export default class entity extends Vue{
  ...
  private addSubNode(i:any){  //新增子节点
    var obj = {
      "id":12,
      "parent_id":i.id,
      "open":true,
  ...
  private addsiblingNode(i:any){  //新增兄弟节点
    var obj = {
      "id":12,
  ...
  private findNode(id:Number,arr:any[]):any{  //通过id寻找节点
  // debugger
    var node;
    arr.map(item=>{
      if(item.id===id){
        ...
        if(item.children) return this.findNode(id,item.children)
      }
    })
    if(node){
      console.log(node)
